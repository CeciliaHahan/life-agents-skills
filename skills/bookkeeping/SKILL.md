...
用 Read 工具读取 `financial-review.xlsx`，了解：
- **共享账户** sheet：最后几行数据（最新日期、当前行数、已有类别）
- **个人账户** sheet：最后几行数据（最新日期、当前行数、已有类别）
...
```python
import openpyxl
wb = openpyxl.load_workbook('financial-review.xlsx', data_only=False)
for name in ['共享账户', '个人账户']:
    ws = wb[name]
...
| 更细类别 | 留空（用户可在确认时补充） |
| 备注 | 商户名或交易描述 |
| 使用者 | 使用者A 或 使用者B（根据截图判断，不确定时问用户） |

**共享账户类别命名规范**（注意 food&drinks 有 s）：
...
wb = openpyxl.load_workbook('financial-review.xlsx')
...
wb.save('financial-review.xlsx')
```

**关键规则**：
- `data_only=False`（默认），保留公式
- 只 append，永不修改已有行
- 不触碰 **收支分析** 和 **budgeting** sheet
...
- 共享账户新增 X 条，日期范围 YYYY/MM/DD - YYYY/MM/DD，合计 ¥XXX
